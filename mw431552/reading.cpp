        ...
        while (global_flag) {
            ...
            for (int u : A) {
                ...
                for (Edge e : current_vertex.edges) {
                    ...
                    MPI_Get(&d_v, 1, MPI_LONG_LONG, owner_rank,
                            local_idx, 1, MPI_LONG_LONG, win_d);
                    ...
                    if (new_d < old_d) {
                        ...
                        MPI_Put(&new_d, 1, MPI_LONG_LONG, owner_rank,
                                local_idx, 1, MPI_LONG_LONG, win_d);
                        ...
                        MPI_Put(&updated, 1, MPI_LONG_LONG, owner_rank,
                                local_idx, 1, MPI_LONG_LONG, win_changed);
...
unordered_map<int, long long> delta_stepping_light_heavy(
    unordered_map<int, Vertex> vertex_mapping,
    int root, int rank, int num_procs, int num_vertices
) {
    int local_vertex_count = vertices_for_rank(rank, num_vertices, num_procs);
    vector<long long> local_d(local_vertex_count, INF);
    vector<long long> local_changed(local_vertex_count, 0);
    ...
    MPI_Barrier(MPI_COMM_WORLD); // Ensure all MPI windows ready

    while (true) {
        // Find next non-empty bucket
        long long local_next_k = -1;
        for (const auto& [bucket_index, bucket_set] : buckets) {
            if (!bucket_set.empty()) {
                if (local_next_k == -1 || bucket_index < local_next_k) {
                    local_next_k = bucket_index;
                }
            }
        }

        long long k;
        MPI_Allreduce(&local_next_k, &k, 1, MPI_LONG_LONG, MPI_MIN, MPI_COMM_WORLD);

        if (k == -1) break; // All buckets empty
        ...
        while (global_flag) {
            ...
            bool temp_flag = !A.empty();
            MPI_Allreduce(&temp_flag, &global_flag, 1, MPI_C_BOOL, MPI_LOR, MPI_COMM_WORLD);
        }

        // --- Process heavy edges once for all vertices in B = bucket[k] ---
        set<int> B = buckets[k];
        for (int u : B) {
            Vertex& current_vertex = vertex_mapping[u];
            long long d_u = local_d[local_index(u, local_vertex_count)];
            ...
            for (Edge e : current_vertex.edges) {
                if (e.weight <= delta) continue; // skip light edges
...
int main(int argc, char** argv) {
    ...
    unordered_map<int, long long> final_values = delta_stepping_light_heavy(my_vertices, global_root, rank, num_processes, num_vertices);
